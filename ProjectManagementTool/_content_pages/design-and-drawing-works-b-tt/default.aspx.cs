...
namespace ProjectManagementTool._content_pages.design_and_drawing_works_b_tt
{
    public partial class _default : System.Web.UI.Page
    {
        ...
        protected void DDlProject_SelectedIndexChanged(object sender, EventArgs e)
        {
            if (DDlProject.SelectedValue != "")
            {
                ...
                if (ds.Tables[0].Rows.Count > 0)
                {
                    ...
                    addDailyProgress.HRef = "/_modal_pages/add-Design_and_drawing_works_b_tt.aspx?ProjectUID=" + DDlProject.SelectedValue + "&WorkPackageUID=" + DDLWorkPackage.SelectedValue;
                }
        ...
        protected void GrdDailyProgress_RowDeleting(object sender, GridViewDeleteEventArgs e)
        {
            ...
            int cnt = getdt.DesignanddrawingworksbttDelete(uid, Session["UserUID"].ToString());
        ...
        protected void btnAdd_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                for (int count = 0; count < GrdPhysicalProgress.Rows.Count; count++)
                {
                    ...
                    TextBox txtCrossing = (TextBox)GrdPhysicalProgress.Rows[count].FindControl("txtCrossing");
                    TextBox txtGfcStatus = (TextBox)GrdPhysicalProgress.Rows[count].FindControl("txtGfcStatus");
                    TextBox txtLength = (TextBox)GrdPhysicalProgress.Rows[count].FindControl("txtLength");
                    TextBox txtPipesReceived = (TextBox)GrdPhysicalProgress.Rows[count].FindControl("txtPipesReceived");
                    TextBox txtStartDepth = (TextBox)GrdPhysicalProgress.Rows[count].FindControl("txtStartDepth");
                    TextBox txtStopDepth = (TextBox)GrdPhysicalProgress.Rows[count].FindControl("txtStopDepth");
                    TextBox txtDiameter = (TextBox)GrdPhysicalProgress.Rows[count].FindControl("txtDiameter");
                    ...
                    int cnt = getdt.InsertorUpdateDesignanddrawingworksbtt(UID, DPR_UID, projectUid, workPackageUid, txtZoneName.Text, txtVillageName.Text, txtCrossing.Text,
                         txtGfcStatus.Text, txtLength.Text, txtDiameter.Text, txtStartDepth.Text, txtStopDepth.Text, txtRemarks.Text, DateTime.Now);
        ...
        private void BindPhysicalProgress()
        {
            ...
            DataSet ds = new DataSet();
            ds = getdt.Getdesignanddrawingworksbtt(DPR_UID, projectUid, workPackageUid);
            if(ds.Tables[0].Rows.Count == 0 && DDLDailyReportMaster.SelectedIndex > 0)
            {
                ...
                DataSet dsPreviousReport = getdt.Getdesignanddrawingworksbtt(DPR_UID_Previous, projectUid, workPackageUid);
        ...
        private void SavePreviousReportData(DataSet ds, Guid desinationUID, Guid projectUid, Guid workPackageUid)
        {
            if (ds != null && ds.Tables[0] != null && ds.Tables[0].Rows.Count > 0)
            {
                foreach (DataRow dataRow in ds.Tables[0].Rows)
                {
                    ...
                    int cnt = getdt.InsertorUpdateDesignanddrawingworksbtt(Guid.NewGuid(), desinationUID, projectUid, workPackageUid, dataRow["ZoneName"].ToString(), dataRow["VillageName"].ToString(), 
                        dataRow["Crossing"].ToString(), dataRow["GFCStatus"].ToString(), dataRow["Length"].ToString(), dataRow["DiaMeter"].ToString(), dataRow["StartDepth"].ToString(), dataRow["StopDepth"].ToString(),
                        dataRow["Remarks"].ToString(), DateTime.Now);
        ...
        protected void btncopy_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                DataSet ds = new DataSet();
                ds = getdt.Getdesignanddrawingworksbtt(sourcemeetingUID, projectUid, workPackageUid);
                if(ds != null && ds.Tables[0] != null && ds.Tables[0].Rows.Count > 0)
                {
                    foreach(DataRow dataRow in ds.Tables[0].Rows)
                    {
                        ...
                        int cnt = getdt.InsertorUpdateDesignanddrawingworksbtt(Guid.NewGuid(), desinationUID, projectUid, workPackageUid, dataRow["ZoneName"].ToString(), dataRow["VillageName"].ToString(),
                      dataRow["Crossing"].ToString(), dataRow["GFC Status"].ToString(), dataRow["Length"].ToString(), dataRow["DiaMeter"].ToString(), dataRow["StartDepth"].ToString(), dataRow["StopDepth"].ToString(),
                      dataRow["Remarks"].ToString(), DateTime.Now);
