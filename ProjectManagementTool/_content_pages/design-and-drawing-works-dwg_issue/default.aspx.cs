...
namespace ProjectManagementTool._content_pages.design_and_drawing_works_dwg_issue
{
    public partial class _default : System.Web.UI.Page
    {
        ...
        
        {
            ...
            else
            {
                if (!IsPostBack)
                {
                    ...
                    Binddesignanddrawingworksbttmaster();
        ...
        protected void DDlProject_SelectedIndexChanged(object sender, EventArgs e)
        {
            if (DDlProject.SelectedValue != "")
            {
                ...
                if (ds.Tables[0].Rows.Count > 0)
                {
                    ...
                    addDailyProgress.HRef = "/_modal_pages/add-Design_and_drawing_works_dwg_issue.aspx?ProjectUID=" + DDlProject.SelectedValue + "&WorkPackageUID=" + DDLWorkPackage.SelectedValue;
                }
        ...
        protected void GrdDailyProgress_RowDeleting(object sender, GridViewDeleteEventArgs e)
        {
            ...
            int cnt = getdt.DesignanddrawingworksdwgissueDelete(uid, Session["UserUID"].ToString());
        ...
        protected void btnAdd_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                for (int count = 0; count < GrdPhysicalProgress.Rows.Count; count++)
                {
                    ...
                    TextBox txtLocation = (TextBox)GrdPhysicalProgress.Rows[count].FindControl("txtLocation");
                    TextBox txtsubmittedbycontractor = (TextBox)GrdPhysicalProgress.Rows[count].FindControl("txtsubmittedbycontractor");
                    TextBox txtsubmitteddate = (TextBox)GrdPhysicalProgress.Rows[count].FindControl("txtsubmitteddate");
                    TextBox txtapproverbyontb = (TextBox)GrdPhysicalProgress.Rows[count].FindControl("txtapproverbyontb");
                    TextBox txtapproverdate = (TextBox)GrdPhysicalProgress.Rows[count].FindControl("txtapproverdate");
                    TextBox txtGFCApprovedByBWSSB = (TextBox)GrdPhysicalProgress.Rows[count].FindControl("txtGFCApprovedByBWSSB");
                    TextBox txtGFCapproverdate = (TextBox)GrdPhysicalProgress.Rows[count].FindControl("txtGFCApprovedDate");
                    ...
                    sDate1 = txtapproverdate.Text;
                    //sDate1 = sDate1.Split('/')[1] + "/" + sDate1.Split('/')[0] + "/" + sDate1.Split('/')[2];
                    sDate1 = getdt.ConvertDateFormat(sDate1);
                    cdate2 = Convert.ToDateTime(sDate1);

                    sDate1 = txtGFCapproverdate.Text;
                    sDate1 = getdt.ConvertDateFormat(sDate1);
                    cdate3 = Convert.ToDateTime(sDate1);


                    //
                    int cnt = getdt.InsertorUpdateDesignanddrawingworksdwgissue(UID, DPR_UID, projectUid, workPackageUid, txtZoneName.Text,txtLocation.Text,Convert.ToDecimal(txtsubmittedbycontractor.Text), CDate1, Convert.ToDecimal(txtapproverbyontb.Text), cdate2,Convert.ToDecimal(txtGFCApprovedByBWSSB.Text),cdate3, DateTime.Now);
        ...
        private void BindPhysicalProgress()
        {
            ...
            DataSet ds = new DataSet();
            ds = getdt.GetdesignanddrawingworksdwgIssue(DPR_UID, projectUid, workPackageUid);
            if (ds.Tables[0].Rows.Count == 0 && DDLDailyReportMaster.SelectedIndex > 0)
            {
                ...
                DataSet dsPreviousReport = getdt.GetdesignanddrawingworksdwgIssue(DPR_UID_Previous, projectUid, workPackageUid);
        ...
        private void SavePreviousReportData(DataSet ds, Guid desinationUID, Guid projectUid, Guid workPackageUid)
        {
            if (ds != null && ds.Tables[0] != null && ds.Tables[0].Rows.Count > 0)
            {
                foreach (DataRow dataRow in ds.Tables[0].Rows)
                {
                    ...
                    int cnt = getdt.InsertorUpdateDesignanddrawingworksdwgissue(Guid.NewGuid(), desinationUID, projectUid, workPackageUid, dataRow["ZoneName"].ToString(), dataRow["Location"].ToString(),
                      Convert.ToDecimal(dataRow["submittedbycontrator"].ToString()), Convert.ToDateTime(dataRow["SubmittedDate"].ToString()), Convert.ToDecimal(dataRow["approverbyontb"].ToString()), Convert.ToDateTime(dataRow["approverdate"].ToString()), Convert.ToDecimal(dataRow["GFCApprovedByBWSSB"].ToString()), Convert.ToDateTime(dataRow["GFCapproveddate"].ToString()), DateTime.Now);
                }
            }
        }
        ...
        private void Binddesignanddrawingworksbttmaster()
        {
        ...
        protected void btncopy_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                DataSet ds = new DataSet();
                ds = getdt.GetdesignanddrawingworksdwgIssue(sourcemeetingUID, projectUid, workPackageUid);
                if(ds != null && ds.Tables[0] != null && ds.Tables[0].Rows.Count > 0)
                {
                    foreach(DataRow dataRow in ds.Tables[0].Rows)
                    {
                        ...
                        int cnt = getdt.InsertorUpdateDesignanddrawingworksdwgissue(Guid.NewGuid(), desinationUID, projectUid, workPackageUid, dataRow["ZoneName"].ToString(), dataRow["Location"].ToString(),
                       Convert.ToDecimal(dataRow["Submitted By contractor"].ToString()), Convert.ToDateTime(dataRow["Submitted Date"].ToString()), Convert.ToDecimal(dataRow["Approvered By ONTB"].ToString()), Convert.ToDateTime(dataRow["Approvered Date"].ToString()), Convert.ToDecimal(dataRow["GFCApprovedByBWSSB"].ToString()), Convert.ToDateTime(dataRow["GFCapproverdate"].ToString()), DateTime.Now);
        ...
        protected void GrdPhysicalProgress_RowDataBound(object sender, GridViewRowEventArgs e)
        {
            if (e.Row.RowType == DataControlRowType.DataRow)
            {
                ...
                TextBox txtGFCApprovedDate = (TextBox)e.Row.FindControl("txtGFCApprovedDate");

                if (txtGFCApprovedDate.Text != "")
                {
                    txtGFCApprovedDate.Text = Convert.ToDateTime(txtGFCApprovedDate.Text).ToString("dd/MM/yyyy");
                }
            }
